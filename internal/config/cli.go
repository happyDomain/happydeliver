...
func declareFlags(o *Config) {
	...
	flag.StringVar(&o.Email.RelayAddr, "relay-addr", o.Email.RelayAddr, "Listen address of the XFORWARD-to-XCLIENT relay, to run behind an MTA that already owns port 25 (empty = disabled)")
	flag.StringVar(&o.Email.RelayUpstream, "relay-upstream", o.Email.RelayUpstream, "Local MTA the relay forwards to")
	flag.Var(&StringArray{&o.Email.RelayTrustedNets}, "relay-trusted-nets", "IP or CIDR allowed to use XFORWARD/XCLIENT on the relay (use this option multiple times to append multiple networks)")
