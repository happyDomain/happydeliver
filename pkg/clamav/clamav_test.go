...
// It answers FOUND when the streamed payload contains infectedMarker,
	...
	go func() {
		for {
			...
			go func(conn net.Conn) {
				...
				switch {
				case bytes.Contains(payload.Bytes(), []byte(infectedMarker)):
...
func TestScanClean(t *testing.T) {
	client := New(fakeClamd(t), 5*time.Second)
...
func TestScanInfected(t *testing.T) {
	client := New("tcp://"+fakeClamd(t), 5*time.Second)

	scan := client.ScanBytes(context.Background(), []byte(infectedMarker))
...
func TestScanSizeLimitError(t *testing.T) {
	client := New(fakeClamd(t), 5*time.Second)
...
func TestScanLargePayloadChunking(t *testing.T) {
	client := New(fakeClamd(t), 5*time.Second)

	// Larger than one 64KB chunk, the marker at the end to prove full streaming
	payload := append(bytes.Repeat([]byte("A"), 3*clamdChunkSize), []byte(infectedMarker)...)
...
func TestScanConnectionRefused(t *testing.T) {
	...
	client := New(address, 1*time.Second)
...
func TestScanContextTimeout(t *testing.T) {
	...
	client := New(listener.Addr().String(), 10*time.Second)
...
func TestNewDisabled(t *testing.T) {
	if client := New("", time.Second); client != nil {
		t.Error("Expected nil client for empty address")
	}
}

func TestNewUnixSocket(t *testing.T) {
	client := New("unix:///run/clamav/clamd.sock", time.Second)
	...
	client = New("/run/clamav/clamd.sock", time.Second)
	if client.network != "unix" {
		t.Errorf("Bare socket path should imply unix network, got %q", client.network)
	}
}

// TestParseClamdReply holds every shape a clamd verdict comes in.
func TestParseClamdReply(t *testing.T) {
	tests := []struct {
		...
		{"stream: OK", "clean", ""},
		{"/some/path: OK", "clean", ""},
		{"stream: Eicar-Signature FOUND", "infected", "Eicar-Signature"},
		{"Win.Test.EICAR_HDB-1 FOUND", "infected", "Win.Test.EICAR_HDB-1"},
		{"INSTREAM size limit exceeded. ERROR", "too_large", ""},
		{"stream: Can't open file ERROR", "error", ""},
	...
	for _, tt := range tests {
		t.Run(tt.reply, func(t *testing.T) {
			scan := parseClamdReply(tt.reply)

			if scan.Status != tt.status || scan.Signature != tt.signature {
				t.Errorf("parseClamdReply(%q) = %+v, want status %q signature %q", tt.reply, scan, tt.status, tt.signature)
			}
			if scan.Status == "error" || scan.Status == "too_large" {
				if scan.Error == "" {
					t.Error("Expected the error to carry the reply")
				}
			}
		})
	}
...
func TestNewTCP(t *testing.T) {
	for address, want := range map[string]string{
		"tcp://clamd.example.com:3310": "clamd.example.com:3310",
		"127.0.0.1:3310":               "127.0.0.1:3310",
	} {
		client := New(address, time.Second)
		if client.network != "tcp" || client.address != want {
			t.Errorf("New(%q): network=%q address=%q, want tcp %q", address, client.network, client.address, want)
		}
	}
}

// TestScanServerHangsUp: a daemon dropping the connection before any verdict
// is an error, not a clean file.
func TestScanServerHangsUp(t *testing.T) {
	listener, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		t.Fatal(err)
	}
	...
	client := New(listener.Addr().String(), time.Second)
	scan := client.ScanBytes(context.Background(), bytes.Repeat([]byte("x"), 4*clamdChunkSize))
