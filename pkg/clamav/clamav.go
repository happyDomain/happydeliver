...
// Package clamav hands a payload to a clamd daemon and says what came back.
// It knows nothing of email: what a verdict is worth is the caller's business.
package clamav
...
// Client talks to a clamd daemon using its native protocol
type Client struct {
...
// Scan is the outcome of scanning one payload with clamd
type Scan struct {
...
// New creates a client for the given clamd address.
...
// scan verdict
func (c *Client) ScanBytes(ctx context.Context, data []byte) *Scan {
	dialer := &net.Dialer{Timeout: c.timeout}
	conn, err := dialer.DialContext(ctx, c.network, c.address)
	if err != nil {
		return &Scan{Status: "error", Error: fmt.Sprintf("failed to connect to clamd: %v", err)}
	}
	defer conn.Close()
	...
	if err := conn.SetDeadline(deadline); err != nil {
		return &Scan{Status: "error", Error: fmt.Sprintf("failed to set deadline: %v", err)}
	}

	if _, err := conn.Write([]byte("zINSTREAM\x00")); err != nil {
		return &Scan{Status: "error", Error: fmt.Sprintf("failed to send INSTREAM command: %v", err)}
	}
	...
	for offset := 0; offset < len(data); offset += clamdChunkSize {
		...
		if _, err := conn.Write(sizeBuf[:]); err != nil {
			return &Scan{Status: "error", Error: fmt.Sprintf("failed to stream data to clamd: %v", err)}
		}
	...
	if _, err := conn.Write(sizeBuf[:]); err != nil {
		return &Scan{Status: "error", Error: fmt.Sprintf("failed to terminate stream: %v", err)}
	}

	reply, err := bufio.NewReader(conn).ReadString('\x00')
	if err != nil && reply == "" {
		return &Scan{Status: "error", Error: fmt.Sprintf("failed to read clamd reply: %v", err)}
	}
...
func parseClamdReply(reply string) *Scan {
	...
	case strings.HasSuffix(reply, " FOUND"):
		...
		return &Scan{Status: "infected", Signature: signature}
	case strings.HasSuffix(reply, "ERROR"):
		if strings.Contains(strings.ToLower(reply), "size limit") {
			return &Scan{Status: "too_large", Error: reply}
		}
		return &Scan{Status: "error", Error: reply}
	default:
		return &Scan{Status: "error", Error: fmt.Sprintf("unexpected clamd reply: %q", reply)}
	}
}
