...
// TestScannerSkipsASymbolWithoutAName: a key rspamd left empty names nothing
// a sender could look up.
func TestScannerSkipsASymbolWithoutAName(t *testing.T) {
	scanner, _ := scannerAgainst(t, replyWith(t,
		`{"symbols":{"":{"name":"","score":1.0},"ZERO_FONT":{"name":"ZERO_FONT","score":1.0}}}`))
	...
	if len(result.Symbols) != 1 {
		t.Errorf("read %d symbol(s), want the one with a name", len(result.Symbols))
	}
}

// TestScannerReadsARejection: is_spam is the action, by the schema's own
// definition.
func TestScannerReadsARejection(t *testing.T) {
	for action, spam := range map[string]bool{
		"reject":      true,
		"soft reject": true,
		"greylist":    false,
		"no action":   false,
		...
		t.Run(action, func(t *testing.T) {
			scanner, _ := scannerAgainst(t, replyWith(t, `{"action":"`+action+`","required_score":0,"symbols":{}}`))
			...
			if result.IsSpam != spam {
				t.Errorf("is_spam is %v for %q, want %v", result.IsSpam, action, spam)
			}
			// A threshold of zero is no threshold.
			if result.Threshold != nil {
				t.Errorf("read a threshold of %v, want none", *result.Threshold)
			}
		})
	}
}

func TestNewScannerDefaultsItsTimeout(t *testing.T) {
	scanner := NewScanner("http://rspamd.example.com:11333/", 0, nil)

	if scanner.client.Timeout != rspamdScanTimeout {
		t.Errorf("a timeout of zero left %v, want the default %v", scanner.client.Timeout, rspamdScanTimeout)
	}
	if scanner.url != "http://rspamd.example.com:11333/checkv2" {
		t.Errorf("the scanner points at %q, want the checkv2 endpoint", scanner.url)
	}
}

// TestFirstLine bounds what a reply that was not a scan contributes to a log
// line.
func TestFirstLine(t *testing.T) {
	tests := map[string]string{
		"Denied\n":                    "Denied",
		"  first line\r\nsecond line": "first line",
		"a reply that runs long":      "a reply th…",
		"":                            "(empty reply)",
		"\n\n":                        "(empty reply)",
	}

	for body, want := range tests {
		if got := firstLine([]byte(body), 10); got != want {
