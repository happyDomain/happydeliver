...
func extractLogotypeSVG(extensionValue []byte) ([]byte, error) {
	...
	// tolerate a raw SVG payload. The inflated size is capped to guard
	// against a decompression bomb hidden in the certificate.
	if reader, err := gzip.NewReader(bytes.NewReader(decoded)); err == nil {
		...
		if err == nil {
			if int64(len(inflated)) > MaxLogoSize {
				return nil, fmt.Errorf("decompressed embedded logo exceeds the maximum allowed size of %d bytes", MaxLogoSize)
			}
			return inflated, nil
		}
	}
