...
func (a *RspamdAnalyzer) AnalyzeRspamd(email *EmailMessage) *model.RspamdResult {
	...
	if !hasAnySpamHeader(headers, "X-Spamd-Result", "X-Rspamd-Score", "X-Spam-Status", "X-Spam-Score", "X-Spam-Flag") {
		return nil
	}
	...
	shaped := readSpamShapedVerdict(headers)
	if !hasScore && shaped.hasScore {
		result.Score = shaped.score
		hasScore = true
	}
	...
	case shaped.hasVerdict:
		result.IsSpam = shaped.isSpam
	}
