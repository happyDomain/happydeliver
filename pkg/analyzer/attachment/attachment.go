...
// credentials is disabled, and the report says so.
...
func (a *Analyzer) Analyze(email *mailmsg.Message) *Results {
	...
	for i := range parts {
		...
		// Every known scanner gets its entry before any of them is asked, so
		// that the file carries a complete answer whatever happens next.
		attachment.Scans = make([]Scan, len(knownScanners))

		for j, info := range knownScanners {
			engine := a.scannerFor(info.Name)

			switch {
			case engine == nil:
				attachment.Scans[j] = Scan{Scanner: info.Name, Status: model.ScanResultStatusDisabled}
			...
			default:
				attachment.Scans[j] = Scan{Scanner: info.Name, Status: model.ScanResultStatusPending}
