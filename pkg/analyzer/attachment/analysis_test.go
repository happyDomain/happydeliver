...
func TestAnalysisCarriesEverythingTheEnginesSaid(t *testing.T) {
	...
	results := &Results{Attachments: []Attachment{{
		...
		Scans: []Scan{
			...
			{Scanner: "clamav", Status: model.ScanResultStatusUnknown},
		},
	}}}
	...
	empty := (*check.Scans)[1]
	if empty.Status != model.ScanResultStatusUnknown || empty.Verdict != nil || empty.EnginesTotal != nil || empty.Link != nil || empty.Detail != nil || empty.Metadata != nil {
