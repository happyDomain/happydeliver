...
func budgetExhausted(location string, budget *archiveBudget) []reading.Finding {
...
func inspectArchive(data []byte, location string, depth int, budget *archiveBudget) (findings []reading.Finding) {
	if budget == nil {
...
func exceededDepth(location string) []reading.Finding {
	return []reading.Finding{finding(
		defectNestedArchive,
		model.IssueTypeNestedArchive,
		model.IssueSeverityMedium,
		location,
		fmt.Sprintf("Archive nesting exceeds %d levels; deeper content was not inspected", archiveMaxDepth),
		"Deeply nested archives are used to evade content scanners",
	)}
}

// inspectMember runs the per-file checks on an extracted archive member and
// recurses into nested archives
func inspectMember(name string, content []byte, location string, depth int, budget *archiveBudget) (findings []reading.Finding) {
	memberLocation := fmt.Sprintf("%s → %s", location, name)

	findings = append(findings, staticFindings(name, "", content, memberLocation)...)
	...
	if isArchive(content) {
		if depth+1 >= archiveMaxDepth {
			...
			findings = append(findings, finding(
				...
				model.IssueSeverityMedium,
				memberLocation,
				fmt.Sprintf("Archive contains a nested archive %q", name),
				"Nested archives are frequently used to hide malicious payloads from scanners",
			))
...
func inspectZip(data []byte, location string, depth int, budget *archiveBudget) (findings []reading.Finding) {
	...
	for _, entry := range reader.File {
		...
		if entry.Flags&0x1 != 0 {
			if !reportedEncrypted {
				...
				findings = append(findings, finding(
					...
					"Password-protected archives are a common malware-delivery technique because scanners cannot inspect them",
				))
			}
			continue
		}
		...
		if entry.CompressedSize64 > 0 && entry.UncompressedSize64/entry.CompressedSize64 > archiveBombRatio &&
			...
			findings = append(findings, finding(
				...
				location,
				fmt.Sprintf("Archive member %q has an extreme compression ratio (possible archive bomb)", entry.Name),
				"Archive bombs expand to enormous sizes to exhaust scanner and system resources",
			))
			continue
		}
		...
		if truncated {
			findings = append(findings, partiallyScanned(entry.Name, location))
		}
...
// partiallyScanned says that a member was read only as far as the budget went.
func partiallyScanned(name, location string) reading.Finding {
	return finding(
		defectScanSkipped,
		model.IssueTypeScanSkipped,
		model.IssueSeverityMedium,
		location,
		fmt.Sprintf("Archive member %q exceeds the scan budget and was only partially scanned", name),
		"Oversized archive members cannot be fully scanned; treat them as unverified",
	)
}
...
func inspectGzip(data []byte, location string, depth int, budget *archiveBudget) (findings []reading.Finding) {
	if budget.remainingBytes <= 0 {
	...
	if truncated {
		findings = append(findings, finding(
			...
			model.IssueSeverityMedium,
			location,
			"Gzip content exceeds the scan budget and was only partially scanned",
			"Oversized archive members cannot be fully scanned; treat them as unverified",
		))
	}
	...
	if int64(len(content)) > int64(len(data))*archiveBombRatio && len(content) > 1<<20 {
		findings = append(findings, finding(
			...
			model.IssueSeverityHigh,
			location,
			"Compressed data has an extreme compression ratio (possible archive bomb)",
			"Archive bombs expand to enormous sizes to exhaust scanner and system resources",
		))
		return findings
	}
...
func inspectTar(data []byte, location string, depth int, budget *archiveBudget) (findings []reading.Finding) {
	...
	for budget.remainingEntries > 0 {
		...
		budget.remainingBytes -= int64(len(content))
		if truncated {
			findings = append(findings, partiallyScanned(header.Name, location))
		}
