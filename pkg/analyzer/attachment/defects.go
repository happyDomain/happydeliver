...
var (
	...
	// is opened: a PDF that runs JavaScript, an HTML attachment that assembles
	// a payload in the browser.
	//
	// It is priced by gravity because the same defect covers what runs by
	// itself and what merely could: an embedded file inside a PDF is worth
	// telling the sender about and worth nothing on the scale.
	...
	// one behind a password, or one nested inside another. Both are charged,
	// and the cap holds an archive that is several times both to two answers.
...
var (
	...
	// larger than the analysis will read, an archive whose budget ran out.
	// Charging for it would bill a sender for our ceilings.
