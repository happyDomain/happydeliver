		...
		Run: func(_ context.Context, in *attachmentInput) ([]reading.Finding, error) {
			...
			case model.ScanResultStatusMalicious:
				found = reading.NewFinding(
					...
					"Remove the file from the message, and check the machine it was produced on",
				)
			...
			case model.ScanResultStatusSuspicious:
				found = reading.NewFinding(
					...
					"Rebuild the file from a clean source, or host it and link to it; a few engines flagging it is enough for some providers to drop the message",
				)
			...
			default:
				// Clean, unknown, pending: none of them is a finding.
				return nil, nil
			}
