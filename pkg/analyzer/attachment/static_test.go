...
func TestStaticCheckHTMLSmuggling(t *testing.T) {
	...
	findings := staticFindings("open-me.html", "text/html", html, "open-me.html")

	found := false
	for _, f := range findings {
		if f.Type == model.IssueTypeScriptContent && f.Severity == model.IssueSeverityHigh {
			found = true
		}
	}
	if !found {
...
func TestStaticCheckShebang(t *testing.T) {
	...
	findings := staticFindings("run.txt", "text/plain", script, "run.txt")

	if types := findingTypes(findings); types[model.IssueTypeScriptContent] == 0 {
...
func TestInspectArchiveNestedZip(t *testing.T) {
	...
	for _, f := range findings {
		if f.Type == model.IssueTypeExecutableContent &&
			strings.Contains(locationOf(f), "outer.zip → inner.zip → payload.pdf.exe") {
			located = true
		}
	}
	if !located {
...
func TestInspectArchiveDepthLimit(t *testing.T) {
	...
	depthFinding := false
	for _, f := range findings {
		if f.Type == model.IssueTypeNestedArchive && strings.Contains(f.Message, "nesting exceeds") {
			depthFinding = true
		}
	}
	if !depthFinding {
...
func TestInspectArchivePasswordProtected(t *testing.T) {
	...
	if types := findingTypes(findings); types[model.IssueTypePasswordProtected] == 0 {
...
func TestInspectArchiveGzipBomb(t *testing.T) {
	...
	if types := findingTypes(findings); types[model.IssueTypeArchiveBomb] == 0 {
