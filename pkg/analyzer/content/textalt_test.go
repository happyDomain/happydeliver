...
// TestTextAlternativeSimilarityOfShortTexts: a text too short to have a
// three-word sequence is compared word by word, which is all there is of it,
// and nothing at all is compared to nothing.
func TestTextAlternativeSimilarityOfShortTexts(t *testing.T) {
	tests := []struct {
		name       string
		text, html string
		want       float64
	}{
		{"same two words", "hello world", "hello world", 1},
		{"one word of two", "hello world", "goodbye world", 0.5},
		{"nothing in common", "hello world", "goodbye moon", 0},
		{"a short text against a long page", "hello world", "hello world and much more besides", 1},
		{"empty text", "", "hello world and much more besides", 0},
		{"empty page", "hello world", "", 0},
		{"both empty", "", "", 0},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := textAlternativeSimilarity(tt.text, tt.html); got != tt.want {
				t.Errorf("textAlternativeSimilarity(%q, %q) = %.2f, want %.2f", tt.text, tt.html, got, tt.want)
			}
		})
	}
}

// TestComparableURL pins how far two URLs are normalised before being called
// the same destination: the scheme and the case of the host, and nothing
// else.
func TestComparableURL(t *testing.T) {
	tests := map[string]string{
		"https://Example.com/Path?b=2&a=1": "example.com/Path?b=2&a=1",
		"http://example.com/path/":         "example.com/path/",
		"  www.example.com/path  ":         "www.example.com/path",
		"mailto:sender@example.com":        "sender@example.com",
		// A URL that does not parse is compared as written, lowercased.
		"http://exa mple.com/Path": "http://exa mple.com/path",
	}

	for rawURL, want := range tests {
		if got := comparableURL(rawURL); got != want {
			t.Errorf("comparableURL(%q) = %q, want %q", rawURL, got, want)
		}
	}
}
