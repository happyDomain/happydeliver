...
var offDomainLinksCheck = contentCheck{
	...
	Run: func(_ context.Context, in *contentInput) ([]reading.Finding, error) {
		...
		return []reading.Finding{{Defect: defectOffDomainLinks, ContentIssue: model.ContentIssue{
			...
			// qualifies everything read off the parts that arrived.
			Message: fmt.Sprintf(
				"None of the %d destinations this message links to belongs to %s: %s",
				len(destinations), sender[0], namedDomains(destinations),
			),
			Advice: utils.PtrTo("Send at least the main call to action through your own domain, or through a subdomain delegated to your provider; filters score a message whose destinations all lie elsewhere as unrelated to the domain it is sent from"),
		}}}, nil
	},
}
