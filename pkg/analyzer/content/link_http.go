...
const (
	...
	// LinkHTTPServerFailure: the destination failed on its own side (5xx),
...
func httpStatusFinding(subject string, status int) (LinkHTTPFinding, bool) {
	...
	case refusesAutomatedClients(status):
		// The destination turned the probe away, not the recipient: nothing
		// to hold against the message, and nothing to say about the page.
...
func (l *LinkCheck) applyProbe(probe urlprobe.Answer, subject string) {
	...
	} else if refusesAutomatedClients(l.Status) {
		l.Warning = fmt.Sprintf("Could not verify link: destination answered %d to the automated request", l.Status)
	}
}
