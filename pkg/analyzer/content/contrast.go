...
func contrastContentFinding(failure contrastFinding) reading.Finding {
	...
	found := reading.NewFinding(
		defectLowContrast,
		model.ContentIssueTypeLowContrast,
		contrastSeverity(failure.Ratio),
		fmt.Sprintf("%q", run.Text),
		message,
		advice,
	)
	// The spam filter reports the extreme of this as R_WHITE_ON_WHITE. Sharing
	// a key lets the two be read as one defect, ours carrying the ratio and the
	// colours it measured, and the filter's agreement noted on it.
	found.Concern = "low_contrast"
