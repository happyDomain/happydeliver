...
// TestNamedDomains spells out what a sentence holds and counts the rest.
func TestNamedDomains(t *testing.T) {
	tests := []struct {
		domains []string
		want    string
	}{
		{[]string{"example.net"}, "example.net"},
		{[]string{"example.net", "example.org", "example.io"}, "example.net, example.org and example.io"},
		{[]string{"a.example", "b.example", "c.example", "d.example", "e.example"}, "a.example, b.example, c.example, d.example and e.example"},
		{[]string{"a.example", "b.example", "c.example", "d.example", "e.example", "f.example"}, "a.example, b.example, c.example, d.example, e.example and 1 other"},
		{[]string{"a.example", "b.example", "c.example", "d.example", "e.example", "f.example", "g.example"}, "a.example, b.example, c.example, d.example, e.example and 2 others"},
	}

	for _, tt := range tests {
		if got := namedDomains(tt.domains); got != tt.want {
			t.Errorf("namedDomains(%v) = %q, want %q", tt.domains, got, tt.want)
		}
	}
}
