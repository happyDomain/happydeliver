...
func TestAnalyzeURLSuspicions_HostObfuscation(t *testing.T) {
	tests := []struct {
		...
		// Characters no domain name is made of.
		{"http://exam(ple).com/", URLSuspicionEncodedHost},
		{"http://exa\\mple.com/", URLSuspicionEncodedHost},
		// A colon that is neither a port nor a bracketed IPv6 literal.
		{"http://example.com:abc/", URLSuspicionEncodedHost},
	}
	for _, tt := range tests {
		t.Run(tt.url, func(t *testing.T) {
			got := kindsOf(analyzeURLSuspicions(tt.url))
...
// TestURLShortenersOverlay holds the loaded set to what this analysis makes of
// the embedded list: the services it adds because upstream does not list them,
// and the branded short links it refuses to count as hiding a destination.
//
// That the list itself is well formed is the shorteners package's business.
func TestURLShortenersOverlay(t *testing.T) {
	known := urlShorteners()

	if len(known) < 1000 {
		t.Errorf("only %d shorteners known, the embedded list looks truncated", len(known))
	}
	...
	for _, host := range brandedShortHosts {
		if _, ok := known[host]; ok {
			t.Errorf("branded short link %q is still counted as a shortener", host)
		}
	}
}

// TestAnalyzeURLSuspicions_NoAuthority: a URL naming no host has no host to
// read anything off.
func TestAnalyzeURLSuspicions_NoAuthority(t *testing.T) {
	for _, u := range []string{"http:///path", "https://", "http://@/path", "http://:8080/"} {
		t.Run(u, func(t *testing.T) {
			got := kindsOf(analyzeURLSuspicions(u))
			for _, kind := range got {
				if kind == URLSuspicionEncodedHost || kind == URLSuspicionIPHost || kind == URLSuspicionNonStandardPort {
					t.Errorf("analyzeURLSuspicions(%q) = %v, want nothing said of a host it does not name", u, got)
				}
			}
		})
	}
