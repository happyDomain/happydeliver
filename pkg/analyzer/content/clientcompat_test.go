...
// TestJoinWithAnd writes a list the way a sentence does, however short.
func TestJoinWithAnd(t *testing.T) {
	tests := []struct {
		...
		{[]string{"Outlook", "Gmail"}, "Outlook and Gmail"},
		{[]string{"Outlook", "Gmail", "Yahoo"}, "Outlook, Gmail and Yahoo"},
	}
...
// TestCSSOriginNames pins what a location calls where a declaration came from.
func TestCSSOriginNames(t *testing.T) {
	tests := map[cssOrigin]string{
		originStyleElement:   "<style> element",
		originStyleAttribute: "style attribute",
		originStylesheetLink: "linked stylesheet",
		cssOrigin(255):       "markup",
	}

	for origin, want := range tests {
		if got := origin.String(); got != want {
			t.Errorf("%d.String() = %q, want %q", origin, got, want)
		}
	}
}

func TestIsWebFontStylesheetOfAnUnparsableHref(t *testing.T) {
	if isWebFontStylesheet("http://exa mple.com/fonts.css") {
		t.Error("a href that does not parse was read as a font")
	}
}
