...
// TestParseInlineDeclarations reads a style attribute the way a client does:
// the last declaration of a property wins, the priority comes off the value,
// and a declaration that cannot be read does not stop the reading of the next.
func TestParseInlineDeclarations(t *testing.T) {
	tests := []struct {
		...
		{"nothing", "   ", nil},
		{"one declaration", "color: #fff", map[string]string{"color": "#fff"}},
		{"the last one wins", "color: red; color: blue", map[string]string{"color": "blue"}},
		{"priority comes off", "color: #1a1a1a !IMPORTANT; font-weight: bold!important", map[string]string{"color": "#1a1a1a", "font-weight": "bold"}},
		{"a vendor prefix comes off", "-webkit-text-size-adjust: none", map[string]string{"text-size-adjust": "none"}},
		{"an unreadable declaration is skipped", "mso-hide:all; ; color: red", map[string]string{"mso-hide": "all", "color": "red"}},
		{"an Outlook hack does not hide what follows", "{color: red}; background-color: white", map[string]string{"background-color": "white"}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := parseInlineDeclarations(tt.style)

			if len(got) != len(tt.want) {
				t.Fatalf("parseInlineDeclarations(%q) = %v, want %v", tt.style, got, tt.want)
			}
			for property, value := range tt.want {
				if got[property] != value {
					t.Errorf("%s reads %q, want %q", property, got[property], value)
				}
			}
		})
	}
...
func TestWithoutImportant(t *testing.T) {
	tests := map[string]string{
		"#fff":                "#fff",
		"#fff !important":     "#fff",
		"#fff!important":      "#fff",
	...
	for value, want := range tests {
		if got := withoutImportant(value); got != want {
			t.Errorf("withoutImportant(%q) = %q, want %q", value, got, want)
		}
	}
}

// TestBackgroundShorthandColor picks the colour out of what else a background
// shorthand carries, in either of the forms a colour is written.
func TestBackgroundShorthandColor(t *testing.T) {
	tests := map[string]string{
		"#fff":                                 "#fff",
		"url(bg.png) no-repeat center #1a1a1a": "#1a1a1a",
		"white url(bg.png)":                    "white",
		"rgb(255, 255, 255)":                   "rgb(255, 255, 255)",
		"url(bg.png) no-repeat":                "",
		"":                                     "",
	}

	for shorthand, want := range tests {
		if got := backgroundShorthandColor(shorthand); got != want {
			t.Errorf("backgroundShorthandColor(%q) = %q, want %q", shorthand, got, want)
		}
	}
}

func TestParseAbsoluteLengthRefusesWhatIsNotANumber(t *testing.T) {
	for _, value := range []string{"largepx", "px", "1.2.3pt"} {
		if size, ok := parseAbsoluteLength(value); ok {
			t.Errorf("%q read as %.2f px, want it refused", value, size)
		}
	}
}

func TestReadsAsBold(t *testing.T) {
	...
	for weight, want := range tests {
		if got := readsAsBold(weight); got != want {
			t.Errorf("readsAsBold(%q) = %v, want %v", weight, got, want)
		}
	}
}

func TestContrastRunsOfNothing(t *testing.T) {
	if runs := contrastRuns(nil); runs != nil {
		t.Errorf("contrastRuns(nil) = %v, want nothing", runs)
	}
}
