...
func TestProbeClassifiesStatusCodes(t *testing.T) {
	...
	for _, status := range []int{404, 410, 401, 403, 500, 503} {
...
func TestProbeRefusedAutomationIsNotBroken(t *testing.T) {
	for _, status := range []int{http.StatusTooManyRequests, 999} {
		t.Run(strconv.Itoa(status), func(t *testing.T) {
			...
			check := probeLink(t, analyzer, server.URL)
			if check.Status != status {
				t.Errorf("Status = %d, want %d", check.Status, status)
			}
			if len(check.HTTPFindings) != 0 {
				t.Errorf("HTTPFindings = %v, want none: the page is there for whoever clicks", findingKinds(check.HTTPFindings))
			}
			if check.Warning == "" {
				t.Errorf("Warning is empty, want the link reported as unverified")
			}
