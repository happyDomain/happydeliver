...
func assertHeaderMap(t *testing.T, name string, got ScannerHeaders, want map[string]string) {
	t.Helper()
...
// TestSpamScannerHeadersParsedEmail checks the routing on a message read from
// the wire, where the header names come from the parser rather than written out
// in canonical form here.
func TestSpamScannerHeadersParsedEmail(t *testing.T) {
...
`
	...
	assertHeaderMap(t, "Rspamd", set.For(ScannerRspamd), nil)
}
...
func TestAnalyzeSpamAssassinRspamdOnlyHost(t *testing.T) {
	...
	rspamd := NewRspamdAnalyzer(nil).AnalyzeRspamd(spamHeadersOf(email, ScannerRspamd))
	if rspamd == nil {
